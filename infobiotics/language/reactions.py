...
vector = r'\(\s*(?P<vector>([-+]?\d+\s*,\s*[-+]?\d+)|(%s)|([*]))\s*\)\s*' % id
...
#        return "%s%s%sreaction(reactants_outside=%r, reactants_inside=%r, reactants_label='%s', products_outside=%r, products_inside=%r, products_label='%s', rate=%s)" % (indent, id, '=' if id != '' else '', self.reactants_outside, self.reactants_inside, self.reactants_label, self.products_outside, self.products_inside, self.products_label, self.rate)
    ...
    def iml(self, indent='', indent_level=0, comment=False, volume=1 * metre ** 3):
        rate = self.rate if volume is None else stochastic_rate_constant(self, volume=volume)
        rate = str(rate.magnitude)
...
#        return reaction(**kwargs)
    ...
    def extract_from_rule(self, rule):
        is_transport_rule = False
        # transport rule must be first because otherwise vector becomes units_unparsed
        if transport_rule_matcher.match(rule):
            match = transport_rule_matcher.match(rule)
            is_transport_rule = True
        elif transformation_rule_matcher.match(rule):
            match = transformation_rule_matcher.match(rule)
        else:
            raise ValueError("Failed to create reaction from rule '%s'.\nAll rules require at least one space before '->'." % rule) #TODO examples of valid rules
...
#                        raise ValueError('rate_id2 ignored if rate_id set otherwise mandatory')
        ...
        if reactants_label is None:
            reactants_label = self.label

        if products_label is None:
            products_label = self.label
...
#            print 'got here too'
    ...
    def has_zeroth_order_rate(self):
        return can_rescale_rate(self.rate, molar * config.time_units ** -1)

    def has_first_order_rate(self):
        return can_rescale_rate(self.rate, config.time_units ** -1)

    def has_second_order_rate(self):
        return can_rescale_rate(self.rate, (molar ** -1) * (config.time_units ** -1))

    def has_equilibrium_dissociation_rate(self):
        return can_rescale_rate(self.rate, molar)


def can_rescale_rate(rate, unitquantity):
    try:
        rate.rescale(unitquantity)
...
def is_zeroth_order_rate(rate):
    return can_rescale_rate(rate, molar * config.time_units ** -1)

def is_first_order_rate(rate):
    return can_rescale_rate(rate, config.time_units ** -1)

def is_second_order_rate(rate):
    return can_rescale_rate(rate, (molar ** -1) * (config.time_units ** -1))

def is_equilibrium_dissociation_rate(rate):
    return can_rescale_rate(rate, molar)


def stochastic_rate_constant(reaction, volume=1 * metre ** 3):
    ...
    if isinstance(reaction.rate, (int, float)):
        k = reaction.rate * config.time_units ** -1
    else:
        k = reaction.rate
...
#        del(k)
    ...
    if reaction.has_zeroth_order_reactants():
        ...
        elif is_equilibrium_dissociation_rate(k):
            # use K_D and k_on_max to calculate k_off 
            k_off = K_D * k_on_max
            c = k_off.rescale(config.time_units ** -1)
        else:
#            print reaction.rate
            raise ValueError("Cannot convert deterministic reaction rate constant '%s' for first order reaction." % k)# '%s'." % (k, reaction))
    elif reaction.has_second_order_homo_reactants():
        if is_second_order_rate(k):
            ...
            config.log.info("Using k_on_max (%s) for rule '%s' with equilibrium rate constant (%s)." % (k_on_max, reaction, k))
        else:
