...
class ProgressMeter(HasTraits):
    ...
    def update(self, value):
        ''' Guarded set progress function. 
        
        Subclasses that override this method should call 'super()':
            super(SubclassOfProgressMeter, self).update(value)
        ...
        # set value
        self.value = value # triggers self._value_changed(value)
        
        # decide if finished
        if value >= self.max:
            self.finished = True # triggers self._finished_fired(True)
            
        return (not self.cancelled, False)
