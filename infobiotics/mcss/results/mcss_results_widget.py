...
#		self._geometry_orig = self.geometry() # remember geometry from Ui_McssResultsWidget
	...
	def closeEvent(self, event):
		...
		event.accept()
#		return super(McssResultsWidget, self).closeEvent(event)
...
#		return previous
	...
	def load_settings(self):
		...
		def restore_selection(checkboxsetting, checkbox, listwidgetsetting, listwidget):
			checked = settings.value(checkboxsetting, Qt.Unchecked, type=bool)
			if checked:
...
#			QMessageBox.warning(self, QString("Error"), QString("There was an error reading %s\n%s") % (filename, e))
			...
			QMessageBox.warning(self, "Error", str(e).replace('`', ''))# + "\nmcss simulated a model 0 rules: please check model is correct.")
		if simulation == None:
			if self.loaded:
		...
		# alert if value of to spin box < simulation.max_time
		...
		if not 0 < to < simulation.max_time:
			...
			#ui_mcss_results_widget.Ui_McssResultsWidget.retranslateUi: self.to_spin_box.setToolTip(QtGui.QApplication.translate("McssResultsWidget", "Select timepoints up to and including <I>n</I>", None, QtGui.QApplication.UnicodeUTF8))
			self.ui.to_spin_box.setToolTip('<div style="color:black">Select timepoints up to and including <I>n</I>.\n</div>\n<div style="color:red">Restored value less than maximum.</div>')
			self.connect(self.ui.to_spin_box, SIGNAL('valueChanged(double)'), lambda: self.ui.to_spin_box.setStyleSheet('color:black'))
			self.connect(self.ui.to_spin_box, SIGNAL('valueChanged(double)'), lambda: self.ui.to_spin_box.setToolTip('Select timepoints up to and including <I>n</I>'))
