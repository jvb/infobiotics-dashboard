...
#			print os.path.splitunc(self._params_file_) #TODO use for paths
		...
		if sys.platform.startswith('win'):
			method = self._perform_windows
		else:
			method = self._perform
#			method = self._perform_windows_fake # testing Windows error handling 
	...
	def _perform_windows_fake(self, expecting_no_output=False):
		'''Using subprocess perform experiment in terminal, showing output. If 
		that fails, it will fail fast and we rerun catching errors.'''
		import subprocess
...
#		os.remove(self.temp_params_file.name) # deletes file on Windows
	...
	def _perform_windows(self, expecting_no_output=False):
		...
		self._subprocess = subprocess.Popen(
			[self.executable, self.temp_params_file.name] + self.executable_kwargs[:],
#			['cmd.exe', '/k', self.executable, self.temp_params_file.name] + self.executable_kwargs[:], # keeps console window open 
			cwd=self.directory,
			startupinfo=si,
