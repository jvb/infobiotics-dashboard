...
def _create_security_attributes(*sids, **kwargs):
    ...
    inherit = True#kwargs.get('inherit', 0)
    ...
    attr.SECURITY_DESCRIPTOR = None#desc
    return attr
...
#            SetHandleInformation(pipe, HANDLE_FLAG_INHERIT, 0)
            SetHandleInformation(pipe, 0, HANDLE_FLAG_INHERIT)
...
def _create_inheritable_one_way_handle_for_the_server_to_read(template):
    sattrs = _create_security_attributes(*sids)
    for i in range(100):
        name = template % random.randint(0, 999999)
        try:
...
def _stub(cmd_name, stdin_name, stdout_name, stderr_name):#, hOutputReadTmp_name):
...
#    SetHandleInformation(cmd_pipe, 1, HANDLE_FLAG_INHERIT, 1)
    SetHandleInformation(cmd_pipe, 1, HANDLE_FLAG_INHERIT)
    stdin_pipe = CreateFile(stdin_name, GENERIC_READ, 0, None,
                            OPEN_EXISTING, 0, None)
#    SetHandleInformation(stdin_pipe, HANDLE_FLAG_INHERIT, 1)
    SetHandleInformation(stdin_pipe, 1, HANDLE_FLAG_INHERIT)
    stdout_pipe = CreateFile(stdout_name, GENERIC_WRITE, 0, None,
                             OPEN_EXISTING, 0, None)
#    SetHandleInformation(stdout_pipe, HANDLE_FLAG_INHERIT, 1)
    SetHandleInformation(stdout_pipe, 1, HANDLE_FLAG_INHERIT)
    stderr_pipe = CreateFile(stderr_name, GENERIC_WRITE, 0, None,
                             OPEN_EXISTING, 0, None)
#    SetHandleInformation(stderr_pipe, HANDLE_FLAG_INHERIT, 1)
    SetHandleInformation(stderr_pipe, 1, HANDLE_FLAG_INHERIT)

#    hOutputWrite = CreateFile(
#        hOutputReadTmp_name, 
#        FILE_WRITE_DATA | SYNCHRONIZE,
#        0,
#        _create_security_attributes(),#*sids),
#        OPEN_EXISTING,
#        FILE_ATTRIBUTE_NORMAL,
#        0#None?
#    )
#    
#    hErrorWrite = DuplicateHandle(GetCurrentProcess(),hOutputWrite,
#        GetCurrentProcess(),0,True,DUPLICATE_SAME_ACCESS)
#    
#    # same thing for clients input?
#    hOutputReadTmp, hOutputReadTmp_name = _create_inheritable_one_way_handle_for_the_server_to_read(winspawn.pipe_template)
#    hOutputRead = DuplicateHandle(GetCurrentProcess(), hOutputReadTmp,
#                                  GetCurrentProcess(), 0, False, DUPLICATE_SAME_ACCESS)
...
class winspawn(spawn):
    ...
    def _spawn(self, command, args=None):
        ...
        cmd_pipe, cmd_name = _create_named_pipe(winspawn.pipe_template, sids)
        stdin_pipe, stdin_name = _create_named_pipe(winspawn.pipe_template, sids)
        stdout_pipe, stdout_name = _create_named_pipe(winspawn.pipe_template, sids)
        stderr_pipe, stderr_name = _create_named_pipe(winspawn.pipe_template, sids)
...
#        CloseHandle(hOutputReadTmp)
        ...
        pycmd = 'import winpexpect; winpexpect._stub(r"%s", r"%s", r"%s", r"%s")' \
                    % (cmd_name, stdin_name, stdout_name, stderr_name)#hOutputReadTmp_name #TODO
        ...
        print 'pid =', GetExitCodeProcess(child_handle)

        print 1
        print ConnectNamedPipe(cmd_pipe)
        print 2
