...
#  Date:   05/20/2005
#  Adapted by Jonathan Blakes 2010/08/19
#-------------------------------------------------------------------------------
...
class KeyBindingEditor(KeyBindingEditor):
    ...
    def init(self, parent):
        self.control = KeyBindingCtrl(self, parent.parent())
...
Binding = Str(event='binding', editor=BasicEditorFactory(klass=KeyBindingEditor)())
...
class KeyBindings (HasPrivateTraits):
    ...
    traits_view = View(
        Group(
            Item(label='Click on an entry field, then press the key to assign. Double-click a field to clear it.'),
            Item('bindings',
                style='custom',
                editor=ListEditor(
                    style='custom',
