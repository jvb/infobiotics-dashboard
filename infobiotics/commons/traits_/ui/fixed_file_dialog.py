...
class CreateDirHandler ( Handler ):
    ...
    message = Str('Enter the full path of the directory to create:')
...
class FileExistsHandler ( Handler ):
    ...
    """
    # The current status message:
    message = Str#('File already exists. Overwrite?')
...
#                       editor = ImageEditor( image = '@icons:dialog-warning' )
    ...
    def handler_ok_changed(self, info):
        """ Handles the user clicking the OK button.
        """
        parent = info.ui.control._parent._ui
        info.ui.dispose(True)
        parent.dispose(True)
...
#                     image = '@icons:folder-new',
    ...
    def object_ok_changed ( self, info ):
        ...
        if self.is_save_file and exists( self.file_name ):
            do_later( self._file_already_exists, info )
        else:
            info.ui.dispose( True )
    ...
    def object_create_changed ( self, info ):
        ...
        handler = CreateDirHandler(dir_name=self.file_name)
        handler.edit_traits(context={'handler':handler,'object':self}, parent=info.ui.control)

    def _file_already_exists(self, info):
        ...
        handler = FileExistsHandler(
            message = ("The file '%s' already exists.\nDo you wish to overwrite it?") % basename(self.file_name),
        )
        handler.edit_traits(context={'handler':handler,'object':self}, parent=info.ui.control)
